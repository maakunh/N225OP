...
- `RssChart` は指定本数に満たないと、有効データの末尾に前回取得時の古いデータが残り、境目の日付セルが `--------`（ハイフンのみ）になることがある。VBAは `IsDashMarker` でこれを検出し、`--------` 以降を読み取らずに打ち切るため、古い残骸はグラフに反映されない
...
| 先物チャート末尾に古いデータが残る | RssChartが指定本数に満たず前回の残骸が残存（境目が `--------`） | `IsDashMarker` が `--------` を検出し以降を除外（最新版は対応済み） |
